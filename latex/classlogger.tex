...
\begin{DoxyCode}
24 \{
25   \textcolor{comment}{//cerr << " adding entry current\_depth " << current\_depth << endl;}
26   \hyperlink{classlog__entry}{log\_entry} new\_entry(name, description, \hyperlink{classlogger_a9d29b49bd318a719a8e85b59eac54fe0}{current\_depth});
27   \hyperlink{classlogger_a46e89697a97bc41a90ec78763bfe4d39}{logger::logs}.push\_back(new\_entry);
28   \textcolor{keywordflow}{if} (\hyperlink{classlogger_aab3c778c044ddf4b1b5fad8f4c230ff9}{logger::verbose})\{
29     \textcolor{keywordtype}{string} s = \textcolor{stringliteral}{""};
30     time\_t tt = system\_clock::to\_time\_t(new\_entry.sys\_t);
31     \textcolor{keywordtype}{char} buffer[80];
32     strftime(buffer, 80, \textcolor{stringliteral}{"%F %r"}, localtime(&tt));
33     \textcolor{keywordflow}{for} (\textcolor{keywordtype}{int} i=1;i<\hyperlink{classlogger_a9d29b49bd318a719a8e85b59eac54fe0}{current\_depth};i++)
34       s += \textcolor{stringliteral}{"|---"};
35     \textcolor{keywordtype}{string} buffer\_str(buffer);
36     s +=  name + \textcolor{stringliteral}{" ("} + description + \textcolor{stringliteral}{") "};
37     s += buffer\_str;
38     \hyperlink{classlogger_aec58cd6310ffb9939f8509daf1fcfbf8}{verbose\_stream} << s << endl;
39   \}
40 \}
\end{DoxyCode}
...
73 
74   vector<float> dur(\hyperlink{classlogger_a46e89697a97bc41a90ec78763bfe4d39}{logs}.size()); \textcolor{comment}{// dur[L] is the duration that entry L takes, meaning the difference
       between L and next[L]}
75   vector<float> block\_dur(\hyperlink{classlogger_a46e89697a97bc41a90ec78763bfe4d39}{logs}.size()); \textcolor{comment}{// the duration of the whole block for each entry, which is the
       difference between}
76   vector<float> block\_percent(\hyperlink{classlogger_a46e89697a97bc41a90ec78763bfe4d39}{logs}.size()); \textcolor{comment}{// the percentage of time each entry takes inside a block}
77   duration<float> diff;
78   \textcolor{keywordtype}{string} s;
79   \textcolor{comment}{//cerr << " logs.size() " << logs.size() << endl;}
80   \hyperlink{classlogger_a8490b3400a43c9c6f3cc3cc122f10996}{report\_stream} << endl;
81   \textcolor{keywordflow}{for} (\textcolor{keywordtype}{int} i=1;i<(\hyperlink{classlogger_a46e89697a97bc41a90ec78763bfe4d39}{logs}.size()-1); i++)\{
82     \textcolor{comment}{//cerr << " i " << i << endl;}
83     \textcolor{comment}{// finding duration[i]}
84     diff = \hyperlink{classlogger_a46e89697a97bc41a90ec78763bfe4d39}{logs}[next[i]].t - \hyperlink{classlogger_a46e89697a97bc41a90ec78763bfe4d39}{logs}[i].t;
85     dur[i] = diff.count();
86     \textcolor{comment}{//cerr << " dur[i] " << dur[i] << endl;}
87     \textcolor{comment}{// finding block\_duration }
88     diff = \hyperlink{classlogger_a46e89697a97bc41a90ec78763bfe4d39}{logs}[next[parent[i]]].t - \hyperlink{classlogger_a46e89697a97bc41a90ec78763bfe4d39}{logs}[parent[i]].t;
89     block\_dur[i] = diff.count();
90     \textcolor{comment}{//cerr << " block\_dur[i] " << block\_dur[i] << endl;}
91     block\_percent[i] = dur[i] / block\_dur[i] * 100;
92     \textcolor{comment}{//cerr << " block\_percent[i] " << block\_percent[i] << endl;}
93     s = \textcolor{stringliteral}{""};
94     \textcolor{comment}{//cerr << " logs[i].depth " << logs[i].depth << endl;}
95     \textcolor{keywordflow}{for} (\textcolor{keywordtype}{int} j=1;j<\hyperlink{classlogger_a46e89697a97bc41a90ec78763bfe4d39}{logs}[i].depth; j++)
96       s += \textcolor{stringliteral}{"|---"};
97     s +=  \hyperlink{classlogger_a46e89697a97bc41a90ec78763bfe4d39}{logs}[i].name + \textcolor{stringliteral}{" ("} + \hyperlink{classlogger_a46e89697a97bc41a90ec78763bfe4d39}{logs}[i].description + \textcolor{stringliteral}{"): "} + to\_string(dur[i]) + \textcolor{stringliteral}{"s "} + \textcolor{stringliteral}{"["} + 
      to\_string(block\_percent[i]) + \textcolor{stringliteral}{"\(\backslash\)%]"} + \textcolor{stringliteral}{" "};
98     \hyperlink{classlogger_a8490b3400a43c9c6f3cc3cc122f10996}{report\_stream} << s << endl;
99   \}
100 
101   \hyperlink{classlogger_a8490b3400a43c9c6f3cc3cc122f10996}{report\_stream} << endl << \textcolor{stringliteral}{" itemized log "} << endl;
102   \textcolor{keywordflow}{for} (map<string, float>::iterator it = \hyperlink{classlogger_a8097637b66086185096030c96a1189ba}{item\_duration}.begin(); it!= 
      \hyperlink{classlogger_a8097637b66086185096030c96a1189ba}{item\_duration}.end(); it++)
103     \hyperlink{classlogger_a8490b3400a43c9c6f3cc3cc122f10996}{report\_stream} << it->first << \textcolor{stringliteral}{" : "} << it->second << endl;
104 \}
\end{DoxyCode}
