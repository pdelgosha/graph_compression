...
\begin{DoxyCode}
620                                                 \{
621   \hyperlink{classmarked__graph__compressed_af58307bfadcaa4c3ca6dd594c2f9b3a9}{clear}(); \textcolor{comment}{// to make sure nothing is stored inside me before reading}
622 
623   \textcolor{comment}{// ==== read n, h, delta}
624   fread(&\hyperlink{classmarked__graph__compressed_a8d841016ddb11cfd33748c8deb6277ba}{n}, \textcolor{keyword}{sizeof} \hyperlink{classmarked__graph__compressed_a8d841016ddb11cfd33748c8deb6277ba}{n}, 1, f);
625   fread(&\hyperlink{classmarked__graph__compressed_af6ff623407b673d08d0cab77b39c2193}{h}, \textcolor{keyword}{sizeof} \hyperlink{classmarked__graph__compressed_af6ff623407b673d08d0cab77b39c2193}{h}, 1, f);
626   fread(&\hyperlink{classmarked__graph__compressed_a8b2aaac68e9332ddc78d88eb60b323a7}{delta}, \textcolor{keyword}{sizeof} \hyperlink{classmarked__graph__compressed_a8b2aaac68e9332ddc78d88eb60b323a7}{delta}, 1, f);
627 
628   \textcolor{keywordtype}{int} int\_in; \textcolor{comment}{// auxiliary input integer}
629   \textcolor{comment}{// ===== read type\_mark}
630   \textcolor{comment}{// read number of types}
631   fread(&int\_in, \textcolor{keyword}{sizeof} int\_in, 1, f);
632   \hyperlink{classmarked__graph__compressed_a86b00223525703e973415cbc9c94da68}{type\_mark}.resize(int\_in);
633   \textcolor{keywordflow}{for} (\textcolor{keywordtype}{int} i=0;i<\hyperlink{classmarked__graph__compressed_a86b00223525703e973415cbc9c94da68}{type\_mark}.size();i++)\{
634     fread(&int\_in, \textcolor{keyword}{sizeof} int\_in, 1, f);
635     \hyperlink{classmarked__graph__compressed_a86b00223525703e973415cbc9c94da68}{type\_mark}[i] = int\_in;
636   \}
637 
638   \textcolor{comment}{// ==== read star\_vertices}
639   \textcolor{comment}{// first, read the frequency.}
640   \hyperlink{classmarked__graph__compressed_a7a4ced4586e2e353f9076bd447df5208}{star\_vertices}.first = vector<int>(2); \textcolor{comment}{// frequency,}
641   \textcolor{comment}{// we read its first index which is number of zeros, and the second is n - the first.}
642   fread(&int\_in, \textcolor{keyword}{sizeof} int\_in, 1, f);
643   \hyperlink{classmarked__graph__compressed_a7a4ced4586e2e353f9076bd447df5208}{star\_vertices}.first[0] = int\_in;
644   \hyperlink{classmarked__graph__compressed_a7a4ced4586e2e353f9076bd447df5208}{star\_vertices}.first[1] = \hyperlink{classmarked__graph__compressed_a8d841016ddb11cfd33748c8deb6277ba}{n} - int\_in;
645 
646   \textcolor{comment}{// the integer representation which is star\_vertices.second}
647   mpz\_inp\_raw(\hyperlink{classmarked__graph__compressed_a7a4ced4586e2e353f9076bd447df5208}{star\_vertices}.second.get\_mpz\_t(), f);
648 
...
947 \textcolor{comment}{      if (j%3 == 0 and j > 0) // we know that the cont part is positive, so no need to add one during
...
482 \textcolor{comment}{      if (j%3 == 0 and j > 0) // we know that these indices are the count part (list is of the form \(\backslash\)theta,
...
570   \textcolor{keywordflow}{for} (it3 = \hyperlink{classmarked__graph__compressed_ae179a4737e6eab905c18a94d44ef64b7}{part\_graph}.begin(); it3 != \hyperlink{classmarked__graph__compressed_ae179a4737e6eab905c18a94d44ef64b7}{part\_graph}.end(); it3++)\{
571     oup <<  it3->first; \textcolor{comment}{// the type}
572     \textcolor{comment}{//fwrite(&int\_out, sizeof int\_out, 1, f);}
573     \textcolor{comment}{//output\_bits += sizeof int\_out;}
574 
575     \textcolor{comment}{// the mpz part}
576     oup << it3->second.first;
577     \textcolor{comment}{//output\_bits += mpz\_out\_raw(f, it3->second.first.get\_mpz\_t());}
578     \textcolor{comment}{// the vector part}
579     \textcolor{comment}{// first its size}
580     oup <<  it3->second.second.size();
581     \textcolor{comment}{//fwrite(&int\_out, sizeof int\_out, 1, f);}
582     \textcolor{comment}{//output\_bits += sizeof int\_out;}
583     \textcolor{comment}{// then element by element}
584     \textcolor{keywordflow}{for}(\textcolor{keywordtype}{int} j=0;j<it3->second.second.size();j++)\{
585       oup <<  it3->second.second[j];
586       \textcolor{comment}{//fwrite(&int\_out, sizeof int\_out, 1, f);}
587       \textcolor{comment}{//output\_bits += sizeof int\_out;}
588     \}
589   \}
590 
591   chunks\_new = oup.chunks();
592   space\_log.push\_back(pair<string, int>(\textcolor{stringliteral}{"partition graphs"}, chunks\_new - chunks));
593   chunks = chunks\_new;
594 
595   
596   \textcolor{keywordflow}{if} (\hyperlink{classlogger_a26812b5ba03f130e8dae3446d5fc032f}{logger::stat})\{
597     *\hyperlink{classlogger_a7db37821f875f2ba3540980b355779f5}{logger::stat\_stream} << endl << endl;
598     *\hyperlink{classlogger_a7db37821f875f2ba3540980b355779f5}{logger::stat\_stream} << \textcolor{stringliteral}{" Number of bytes used for each part "} << endl;
599     *\hyperlink{classlogger_a7db37821f875f2ba3540980b355779f5}{logger::stat\_stream} << \textcolor{stringliteral}{" ---------------------------------- "} << endl << endl;
600 
601     \textcolor{keywordtype}{int} total\_chunks = 0;
602     \textcolor{keywordflow}{for} (\textcolor{keywordtype}{int} i=0; i < space\_log.size(); i++)
603       total\_chunks += space\_log[i].second;
604 
605     \textcolor{keywordflow}{for} (\textcolor{keywordtype}{int} i=0; i < space\_log.size(); i++)\{
606       \textcolor{comment}{// each chunks is 4 bytes. }
607       *\hyperlink{classlogger_a7db37821f875f2ba3540980b355779f5}{logger::stat\_stream} << space\_log[i].first << \textcolor{stringliteral}{" -> "}  << 4 * space\_log[i].second <
      < \textcolor{stringliteral}{" ( "} << float(100) * float(space\_log[i].second) / float(total\_chunks) << \textcolor{stringliteral}{" % "} << endl;
608     \}
609 
610     *\hyperlink{classlogger_a7db37821f875f2ba3540980b355779f5}{logger::stat\_stream} << \textcolor{stringliteral}{" Total number of bytes wrote to the output = "} << 4 * 
      total\_chunks << endl;
611   \}
612   
613   oup.close();
614   \hyperlink{classlogger_a9d29b49bd318a719a8e85b59eac54fe0}{logger::current\_depth}--;
615 \}
\end{DoxyCode}
