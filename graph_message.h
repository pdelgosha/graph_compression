...
/*!
  This graph has a reference to a marked_graph object for which we perform message passing to find edge types. The edge types are discovered up to depth h-1, and with degree parameter Delta, where h and Delta are member objects.
...
public:
  ...
  vector<vector<int> > message_list; //!< the list of messages present in the graph, stored in an order consistent with message_dict, i.e. for a message m, if messsage_dict[m] = i, then message_list[i] = m.
...
public:
  ...
  vector<vector<int> >  ver_type; //!< vertex mark and the colored degree matrix of each vertex. For a vertex v,  D[v] is a vector of size \f$1 + L \times L\f$, where the first entry is the vertex mark, and the rest is the colored degree matrix row by row. Here, \f$L\f$ denotes the number of colors. 
  ...
  vector<vector<int> > ver_type_list; //!< the list of all distinct vertex types, obtained from the ver_type array. This is constructed in such a way that ver_type_list[ver_type_dict[x]] = x
